...
from collective.conference.config import REVIEW_STATE
from collective.conference.utils import fmt_price
...
class View(grok.View):
    ...
    def registrations_enabled(self):
        ...
        if state == 'opened':
            return True
        return mt.checkPermission('collective.conference: Add Registration',
                                  self.context)
    ...
    def registrations(self):
        ''' List registrations'''
        ct = self._ct
        results = ct.searchResults(portal_type='collective.conference.registration',
                                   sort_on='created',
                                   sort_order='reverse',
                                   path=self._path)
    ...
    def attendees(self):
        ''' List attenddees'''
        ct = self._ct
        results = ct.searchResults(portal_type='collective.conference.attendee',
                                   sort_on='created',
                                   sort_order='reverse',
                                   path=self._path)
